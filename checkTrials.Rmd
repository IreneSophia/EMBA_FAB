...
df.fab = list.files(path = dt.path, pattern = "FAB-BV_*", full.names = T) %>%
  ...
  mutate(
    key = as.numeric(key), 
    fln = substr(fln, nchar(fln)-27, nchar(fln)-18)
  ) %>%
  group_by(subID, fln) %>%
  summarise(
    trls.fab = n(),
...
df.fab %>% filter(trls.fab == 432 & rsps.fab > 1 & trls.fab == trls_eye.fab)
...
# how many full datasets do we have?
df.fer %>% filter(trls.fer == 64 & rsps.fer == 4 & trls.fer == trls_eye.fer)
...
# how many full datasets do we have?
df.ctr %>% filter(trls.ctr == 16 & rsps.ctr == 4 & trls.ctr == trls_eye.ctr)
...
# how many full datasets do we have?
df.pal %>% filter(trls.pal == 336 & rsps.pal > 1 & trls.pal == trls_eye.pal)
...
# how many full datasets do we have?
df.vmm %>% filter(((trls.vmm == 1190 & runs.vmm == 1) | (trls.vmm == 2380 & runs.vmm == 2)) & trls.vmm == trls_eye.vmm & !is.na(trls_eye.vmm))
