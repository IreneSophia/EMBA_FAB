...
fl.path = '/home/iplank/Documents/EMBA'
...
df.vmm_eye = list.files(path = dt.path, pattern = "*\\.msg$", full.names = T) %>%
  ...
  mutate(
    ...
    subID = as.factor(case_when(
                    substr(subID,2,2) == "_" ~ substr(subID,3,12),
...
# join all the data
df.ls = list(df.fab, df.fer, df.ctr, df.pal)
df = df.ls %>% reduce(full_join, by = "subID") %>%
  mutate(
    subID = case_when(subID == "MT3KYV0QQR" ~ "MT3KYVOQQR",
                      subID == "V0QST6VC93" ~ "VOQST6VC93",
                      TRUE ~ subID)
  )
  

df = merge(df, df.vmm, all.x = T)
