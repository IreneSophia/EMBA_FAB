...
df.que = df.fab.full %>%
  ...
  select(subID, diagnosis, overall, fab, ASRS_total, RAADS_total)
...
if (file.exists("RT_rho.rds")) {
  rhoSamples.rt = readRDS("RT_rho.rds")
} else {
  set.seed(5478)
  rhoSamples.rt = 
    spearmanGibbsSampler(xVals = df.que$overall,
                         yVals = df.que$fab, 
                         nSamples = 5e3)
  saveRDS(rhoSamples.rt, file = "RT_rho.rds")
}
