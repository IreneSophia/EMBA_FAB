...
# S1.4 Explorative analysis of subject-specific FAB with RADS and ASRS

```{r que_fab, fig.height=3}

# merge with the questionnaire values
df.que = df.fab.full %>%
  group_by(subID, diagnosis, stm, cue, ASRS_total, RAADS_total) %>%
  ...
  mutate(
    fab = object - face
  ) %>% group_by(subID, diagnosis, ASRS_total, RAADS_total) %>%
  # calculate the mean FAB per person
  summarise(
    fab = mean(fab)
  ) %>% ungroup() %>%
  select(subID, diagnosis, fab, ASRS_total, RAADS_total)
...
if (file.exists("ASRS_rho.rds")) {
  rhoSamples.asrs = readRDS("ASRS_rho.rds")
} else {
  set.seed(5468)
  rhoSamples.asrs = 
    spearmanGibbsSampler(xVals = df.que$ASRS_total,
                         yVals = df.que$fab, 
                         nSamples = 5e3)
  saveRDS(rhoSamples.asrs, file = "ASRS_rho.rds")
}
if (file.exists("RADS_rho.rds")) {
  rhoSamples.rads = readRDS("RADS_rho.rds")
} else {
  set.seed(5468)
  rhoSamples.rads = 
    spearmanGibbsSampler(xVals = df.que$RAADS_total,
                         yVals = df.que$fab, 
                         nSamples = 5e3)
  saveRDS(rhoSamples.rads, file = "RADS_rho.rds")
}
...
Furthermore, Bayesian Spearman correlations revealed `r interpret_bf(asrs.bf)` associations between FAB and questionnaires assessing ASD (RADS: log(*BF*) = `r round(log(rads.bf),2)`) or ADHD (ASRS: log(*BF*) = `r round(log(asrs.bf),2)`). 
