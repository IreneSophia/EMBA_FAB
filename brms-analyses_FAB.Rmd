...
# fit the model
set.seed(2424)
m.fabd = brm(fab.d ~ diagnosis, data = df.fabd, prior = priors,
            iter = iter, warmup = warm,
            backend = "cmdstanr", threads = threading(8),
            file = "m_fab_cohd"
            )
...
df.ms = as.data.frame(
  fitted(m.fabd, summary = F, 
               ...
               re_formula = NA))
colnames(df.ms) = levels(df.fabd$diagnosis)

df.ms = df.ms %>%
  mutate(
    overall = (ADHD + ASD + BOTH + COMP)/4
  )

```

Similarly to our full model, this model suggests that subject-specific FAB is only attenuated in the ADHD group (ADHD versus COMP: *estimate* = `r round(e1$hypothesis$Estimate,2)` [`r round(e1$hypothesis$CI.Lower,2)`, `r round(e1$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(e1$hypothesis$Post.Prob*100,2)`%; ASD versus COMP: *estimate* = `r round(e2$hypothesis$Estimate,2)` [`r round(e2$hypothesis$CI.Lower,2)`, `r round(e2$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(e2$hypothesis$Post.Prob*100,2)`%; ADHD+ASD versus COMP: *estimate* = `r round(e3$hypothesis$Estimate,2)` [`r round(e3$hypothesis$CI.Lower,2)`, `r round(e3$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(e3$hypothesis$Post.Prob*100,2)`%). Specifically, the model predicts an average effect size across groups of `r round(mean(df.ms$overall),3)` [`r round(ci(df.ms$overall)$CI_low, 3)`, `r round(ci(df.ms$overall)$CI_high, 3)`], with a predicted effect of `r round(mean(df.ms$ADHD),3)` [`r round(ci(df.ms$ADHD)$CI_low, 3)`, `r round(ci(df.ms$ADHD)$CI_high, 3)`] for the ADHD group, `r round(mean(df.ms$ASD),3)` [`r round(ci(df.ms$ASD)$CI_low, 3)`, `r round(ci(df.ms$ASD)$CI_high, 3)`] for the ASD group, `r round(mean(df.ms$BOTH),3)` [`r round(ci(df.ms$BOTH)$CI_low, 3)`, `r round(ci(df.ms$BOTH)$CI_high, 3)`] for the ADHD+ASD group and `r round(mean(df.ms$COMP),3)` [`r round(ci(df.ms$COMP)$CI_low, 3)`, `r round(ci(df.ms$COMP)$CI_high, 3)`] for the COMP group. All of these effects are considered to be `r effectsize::interpret_cohens_d(fixef(m.fabd)[1,1], rules = "gignac2016")` according to Gignac and Szodorai (2016). 
