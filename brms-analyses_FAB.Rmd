...
kable(merge(
  df.sac %>% select(subID, diagnosis) %>% distinct() %>% group_by(diagnosis) %>% summarise(`nos eye tracking` = n()),
  rbind(df.fab, df.exp) %>% select(subID, diagnosis) %>% distinct() %>% group_by(diagnosis) %>% summarise(`nos behavioural` = n())
  ))
...
h1a = hypothesis(m.fab, "0 < (diagnosis1:cue1 + diagnosis2:cue1 + diagnosis3:cue1 - cue1)")
h1a

# H1b: ADHD slower than COMP
h1b = hypothesis(m.fab, "0 < 2*diagnosis1 + diagnosis2 + diagnosis3")
h1b

# H1c: ASD slower than COMP
h1c = hypothesis(m.fab, "0 < 2*diagnosis2 + diagnosis1 + diagnosis3")
h1c
...
h1d
# ... compared to COMP
h1d_comp = hypothesis(m.fab, "0 < 2*diagnosis2:cue1 + diagnosis1:cue1 + diagnosis3:cue1")
h1d_comp
...
h1e
# ... compared to COMP
h1e_comp = hypothesis(m.fab, "0 > 2*diagnosis1:cue1 + diagnosis2:cue1 + diagnosis3:cue1", alpha = 0.025)
h1e_comp
...
e4 = hypothesis(m.fab, "0 < -cue1 - diagnosis3:cue1", alpha = 0.025)
e4
...
# equivalence tests
e_equ = equivalence_test(df.m.fab %>% 
...
Furthermore, our exploration of the data revealed a credible face attention bias across all groups (CI of object - face: `r round(quantile(df.ms$FAB, 0.025)[[1]], 2)` to `r round(quantile(df.ms$FAB, 0.975)[[1]], 2)`ms, posterior probability = `r round(e1$hypothesis$Post.Prob*100, 2)`%). Last, we explored differences between our clinical groups. While an equivalence test was undecided on differences in overall reaction times between adults with ASD and adults with ADHD in our data ([!ADD]), there was a credible difference between face attention bias in the clinical groups (CI of ASD(FAB) - ADHD(FAB): `r round(quantile(df.ms$e5, 0.025)[[1]], 2)` to `r round(quantile(df.ms$e5, 0.975)[[1]], 2)`ms, posterior probability = `r round(e5$hypothesis$Post.Prob*100, 2)`%). [!ADD exploratory fourth group]
