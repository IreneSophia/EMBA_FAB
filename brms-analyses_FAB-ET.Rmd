...
ggplot(dd, aes(x = x, y = y)) + 
  ...
  geom_ribbon(data = subset(dd, x <= points$minima[idx]), aes(ymax = y), ymin = 0, 
              fill = custom.col[1], colour = NA, alpha = .8) +
  geom_ribbon(data = subset(dd, x >= points$minima[idx]), aes(ymax = y), ymin = 0, 
...
h2a = hypothesis(m.cnt, "0 > dir_face1 - diagnosis1:dir_face1 - diagnosis2:dir_face1 - diagnosis3:dir_face1")
h2a
...
e2 = hypothesis(m.cnt, "0 > diagnosis1 - diagnosis3", 
                alpha = 0.025)
e2

# explore difference in saccades between ASD and BOTH
e3 = hypothesis(m.cnt, "0 > diagnosis2 - diagnosis3", 
                alpha = 0.025)
e3

# check equivalence of the groups that seem similar
equ.cnt =
  equivalence_test(df.m.cnt %>% 
...
hypothesis(m.cnt, "0 > dir_face1 - diagnosis1:dir_face1 - diagnosis2:dir_face1 - diagnosis3:dir_face1", 
           alpha = 0.025)
...
hypothesis(m.cnt, "0 < 2*diagnosis1 + diagnosis2 + diagnosis3", alpha = 0.025)

# exploration: ADHD(face) > ADHD(object)
hypothesis(m.cnt, "0 > dir_face1 + diagnosis1:dir_face1", alpha = 0.025)

# exploration: ASD > COMP
hypothesis(m.cnt, "0 < 2*diagnosis2 + diagnosis1 + diagnosis3", alpha = 0.025)

# exploration: BOTH > COMP
hypothesis(m.cnt, "0 < diagnosis2 + diagnosis1 + 2*diagnosis3", alpha = 0.025)
...
hypothesis(m.cnt, "0 > dir_face1 + diagnosis3:dir_face1", alpha = 0.025)
