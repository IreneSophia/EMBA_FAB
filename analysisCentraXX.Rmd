...
write_csv(df.sub %>% filter(is.na(MWT_iq)) %>% select(PID), file = paste(dt.path, "PID_iq_missing.csv", sep = "/"))

# load csv with PIDs and IQs from other studies
df.iqs = read_csv(file = paste(dt.path, "PID_iq.csv", sep = "/"), show_col_types = F) %>%
