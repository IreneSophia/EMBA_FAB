...
df.tsk = list.files(path = dt.path, pattern = "PAL-BV_*", full.names = T) %>%
  ...
  mutate(
    ...
    # add the phases: pre-volatile, volatile and post-volatile based on trial number
    phase = case_when(
      trl <= 72 ~ "prevolatile",
      trl > 72 & trl <= 264 ~ "volatile",
      trl > 264 ~ "postvolatile"
    )
  ) %>% 
  ...
  select(subID, ut, noise, rt, rtc, acc, use, expected, emotion, pic_num, phase) 
